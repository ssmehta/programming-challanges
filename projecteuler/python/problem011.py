...
What is the greatest product of four numbers in any direction (up, down, left,
...
grid = [list(map(int, x.split())) for x in data]
...
# Left Diagonals - answer lies in this portion
for i in range(4, n):
    for j in range(n - 3):
        p = grid[i][j] * grid[i - 1][j + 1] * grid[i - 2][j + 2] * grid[i - 3][j + 3]
        if p > product:
